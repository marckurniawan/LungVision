...
def train_logistic():
    ...
    model_base = make_pipeline(StandardScaler(), LogisticRegression(random_state=42, max_iter=1000))
    ...
    joblib.dump(model_base, os.path.join(MODELS_DIR, "logistic_regression.pkl"))
    evaluate_and_plot("Logistic Regression", y_test, y_pred_base, y_prob_base, is_fuzzy=False)
    
    
    print("\n" + "="*50)
    print("[TRAIN] Logistic Regression - FUZZY")
    print("="*50)
    
    X_train_fuzzy = apply_fuzzification(X_train_raw)
    X_test_fuzzy = apply_fuzzification(X_test_raw)
    
    model_fuzzy = make_pipeline(StandardScaler(), LogisticRegression(random_state=42, max_iter=1000))
    ...
    joblib.dump(model_fuzzy, os.path.join(MODELS_DIR, "logistic_regression_fuzzy.pkl"))
    evaluate_and_plot("Logistic Regression", y_test, y_pred_fuzzy, y_prob_fuzzy, is_fuzzy=True)
